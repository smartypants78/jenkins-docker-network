...
if [[ "$RUN_AS_ROOT" == "yes" ]]; then
  RUN_AS_USER=root
  usermod -d /var/jenkins_home/agent/ root
fi
...
if [ ! -e /var/jenkins_home/agent/.ssh/id_rsa.pub ]; then
  gosu jenkins ssh-keygen -q -N "" -f /var/jenkins_home/agent/.ssh/id_rsa
  echo "Jenkins Slave SSH public key is:"
  echo "================================================================================"
  echo "`cat /var/jenkins_home/agent/.ssh/id_rsa.pub`"
  echo "================================================================================"
fi

if [ "$1" = "java" ]; then
  exec gosu $RUN_AS_USER java $JAVA_OPTS -jar /bin/swarm-client.jar -fsroot /var/jenkins_home/agent/ -labels "$JENKINS_LABELS" $PARAMS
fi
